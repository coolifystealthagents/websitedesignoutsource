...
for (const entry of manifest.entries) {
  ...
  const expectedTime = `<time dateTime="${entry.renderedDate}">${publicationDateFormatter.format(new Date(`${entry.renderedDate}T00:00:00Z`))}</time>`;
  if (!html.includes('datePublished') || !html.includes(expectedTime)) throw new Error(`Rendered date audit failed: ${entry.slug}`);
}
