---
title: "Write a DNS Rollback Runbook for an Outsourced Website Launch"
description: "A step-by-step DNS rollback runbook for website owners who need clear triggers, approved record values, and evidence during a vendor-led launch."
slug: "outsourced-website-dns-rollback-runbook"
published: "2026-09-23"
image: "/illustrations/getillustrations/inkdex-saas/filipino-web-design-production.webp"
---

# Write a DNS Rollback Runbook for an Outsourced Website Launch

A step-by-step DNS rollback runbook for website owners who need clear triggers, approved record values, and evidence during a vendor-led launch. The goal is to restore the last approved public destination when the new website fails a launch-critical check. That outcome needs more than a verbal assurance from a vendor. It needs an owner, an agreed procedure, observable evidence, and a handoff that the company can use after the project ends.

Telling a team to roll back without a prepared value, authorized operator, and verification sequence can extend an outage or accidentally change unrelated services. A useful acceptance process exposes that risk before it becomes an urgent production problem. It also keeps the brief proportionate: the owner defines the business outcome and evidence, while the specialist chooses an implementation that fits the actual platform.

## Start with the customer journey

Describe DNS rollback planning through the visitor or operator task it protects. Name the production route, the action a person takes, the result they should receive, and the system or person responsible for that result. Avoid treating a screenshot, a vendor dashboard, or a green automated check as the whole journey. Those items can be evidence, but only when they connect to the outcome the company intends to accept.

Write down which failures would stop launch, which could be corrected during an agreed follow-up period, and which are observations rather than defects. This prevents the outsourced team from guessing at business priority. It also prevents the company from changing the acceptance rule after seeing the result. the launch commander named by the company should approve the rule and remain able to explain it without relying on private vendor knowledge.

## Gather the inputs before implementation

Prepare these inputs in a company-controlled project record:

- approved pre-launch record values
- critical user journeys and failure thresholds
- authorized decision maker and DNS operator
- old-host availability and observation window
...
Use a one-page DNS rollback runbook as the source of truth for this part of the handoff. Give it a version or update time, the relevant environment, the implementation owner, the acceptance owner, and links to the exact routes or provider records under review. Separate planned values from observed results. A future operator should be able to tell what the team intended, what it actually tested, and what remained open.
...
Before launch, the team exports the approved DNS state and writes the exact record that will be restored. The rollback trigger is not vague dissatisfaction. It is a failed critical path, such as the primary hostname returning an error after the agreed observation period with no safe application fix. The launch commander authorizes the change, the DNS operator restores only the web record, and observers verify both the restored site and services that were meant to remain unchanged.
...
1. Write objective rollback triggers before the change window.
2. Preserve exact record names, types, values, proxy state, and TTL context.
3. Do not roll back mail or verification records as part of a web-only reversal.
4. Record authorization and the actual edit time.
5. Verify the recovered destination from outside the operator network.
...
[Google Public DNS troubleshooting guidance](https://developers.google.com/speed/public-dns/docs/troubleshooting) is a useful reference for this brief. Use resolver tools as evidence from more than one vantage point, while treating the authoritative provider as the source of the configured record. An authoritative source can explain a standard or platform behavior, but it cannot choose the company owner, risk tolerance, customer path, or acceptance threshold.
...
[Use this related outsourced website guide](/blog/website-design-launch-readiness-review)

[Continue with this acceptance and handoff guide](/blog/website-design-launch-readiness-review)
...
[Related guide: project evidence and ownership](/blog/website-design-launch-readiness-review)

[Related guide: implementation and acceptance](/blog/website-design-launch-readiness-review)
