...
export function Footer() {
  return <footer className="footer legit-footer studio-footer"><div className="footer-grid"><div className="footer-brand"><a className="footer-logo" href="/" aria-label={`${site.brand || domain} home`}><LogoMark /></a><p>{site.brand || domain} is a practical guide and project-intake site for agencies considering outsourced website design, production, and QA.</p><p className="footer-note">This is an independent informational website. Submitted requests may be routed to an appropriate design or production partner for follow-up.</p></div><div><h3>Explore</h3><div className="footer-links">{footerItems.map((item: any, idx: number) => <a href={`/services/${getSlug(item)}`} key={idx}>{getTitle(item)}</a>)}</div></div><div><h3>Company</h3><div className="footer-links"><a href="/contact">Contact</a><a href="/blog">Guides</a><a href="/privacy">Privacy Policy</a><a href="/terms">Terms & Conditions</a><a href="/cancellation-policy">Cancellation Policy</a></div></div></div><div className="footer-bottom"><span>© {year} {site.brand || domain}. All rights reserved.</span><span>{domain}</span></div></footer>;
}
